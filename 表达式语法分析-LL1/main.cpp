...
enum Tag
{
    NUM=256,REAL,ID,IF,ELSE,FOR,WHILE,DO,BREAK,CONTINUE,MINUS,AND,OR,EQ,NE,GE,LE,STR,TYPE,KEY,SHARP,EXPR,TERM,FACTOR,EX1PR,TE1RM
};
const int k_w_len=7;
const int t_w_len=5;
...
int main()
{
    ...
    else
    {
        ...
        while(true)
        {
            int tag=Token_list[cur_token]->get_tag();
            if(Stack.top()==EXPR&&(tag==NUM||tag==REAL||tag==ID||tag=='('))
            {
                Stack.pop();
                Stack.push(EX1PR);
                Stack.push(TERM);
            }
            else if(Stack.top()==EX1PR&&(tag=='+'||tag=='-'))
            {
                Stack.pop();
                Stack.push(EX1PR);
                Stack.push(TERM);
                tag=Token_list[++cur_token]->get_tag();
            }
            else if(Stack.top()==EX1PR&&(tag==')'||tag==SHARP))
            {
                Stack.pop();
            }
            else if(Stack.top()==TERM&&(tag==NUM||tag==REAL||tag==ID||tag=='('))
            {
                Stack.pop();
                Stack.push(TE1RM);
                Stack.push(FACTOR);
            }
            else if(Stack.top()==TE1RM&&(tag=='*'||tag=='/'))
            {
                Stack.pop();
                Stack.push(TE1RM);
                Stack.push(FACTOR);
            ...
            else if(Stack.top()==TE1RM&&(tag=='+'||tag=='-'||tag==SHARP||tag==')'))
            {
                Stack.pop();
            }
            else if(Stack.top()==FACTOR&&(tag==NUM||tag==REAL||tag==ID))
            {
                Stack.pop();
                tag=Token_list[++cur_token]->get_tag();
            }
            else if(Stack.top()==FACTOR&&tag=='(')
            {
                Stack.pop();
                Stack.push(')');
                Stack.push(EXPR);
            ...
            else if(Stack.top()==SHARP&&tag==SHARP)
            {
                break;
            }
